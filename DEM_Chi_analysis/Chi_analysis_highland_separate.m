...
DEM50 = GRIDobj('C:\Users\Omri\Desktop\University\MSc\Matlab\DEMc_raw.tif');
...
ST50_2 = load('C:\Users\Omri\Desktop\University\MSc\Matlab\DEM_Chi_analysis\STREAMobjs\ST_TAN_highland.mat');
ST50_2 = ST50_2.ST_TAN_main;

% extract cliff network and higland network
ST50_east = klargestconncomps(ST50_2,1); % assuming the relevant channels are among the longest 3
ST50_west = modify(ST50_2,'rmnodes',ST50_east);

figure(1)
subplot(1,2,1)
imageschs(DEM50);
hold on
plot(ST50_east,'color',[0, 0.4470, 0.7410]);
plot(ST50_west,'color',[1, 0.55, 0.15]);


subplot(1,2,2)
plotdz(ST50_east,DEM50, 'color',[0, 0.4470, 0.7410]);
hold on
plotdz(ST50_west,DEM50, 'color',[1, 0.6940, 0.1250]);
legend('Highland east network', 'Highland west network');

sgtitle(site_name + ' - Map and profile of highland networks');


%%
mn = 0.1:0.05:0.5; % define concavity index range
...
Chi_val_hl = zeros(numel(ST50_east.IXgrid),numel(mn)); % Chi-coordinates for each point (rows) for each m/n ratio (columns)
Chi_val_cl = zeros(numel(ST50_west.IXgrid),numel(mn)); % Chi-coordinates for each point (rows) for each m/n ratio (columns)
figure(2)
for i = 1:numel(mn)
    Chi_val_hl(:,i) = chitransform(ST50_east,A50, 'mn', mn(i)); % Chi-transform for the specific m/n ratio; A0 is by default 1e6 m
    Chi_val_cl(:,i) = chitransform(ST50_west,A50, 'mn', mn(i)); % Chi-transform for the specific m/n ratio; A0 is by default 1e6 m
    
    subplot(3, 3, i)
%     set(gca,'Color',[0.9 0.9 0.9])
    hold on
    scatter(Chi_val_hl(:,i),DEM50.Z(ST50_east.IXgrid), 10, [0, 0.4470, 0.7410], '.'); % plot Chi-Z graph

    scatter(Chi_val_cl(:,i),DEM50.Z(ST50_west.IXgrid), 10, [0.9290, 0.6940, 0.1250], '.'); % plot Chi-Z graph
    grid on
    xlabel('Chi [m]');
    ylabel('Z [m ASL]');
    ...
    Z_hl = DEM50.Z(ST50_east.IXgrid);
    Z_cl = DEM50.Z(ST50_west.IXgrid);
    for j = 1:bins
    ...
    text(min(xlim), max(ylim)-1, sprintf('Weighted mean STD, east = %0.2f', mean_std_all_bins_hl(i)), 'Horiz','left', 'Vert','top', 'Color',[0, 0.4470, 0.7410], 'FontWeight', 'Bold');
    text(min(xlim), max(ylim)-1, sprintf('Weighted mean STD, west = %0.2f', mean_std_all_bins_cl(i)), 'Horiz','left', 'Vert','bottom', 'Color',[0.9290, 0.6940, 0.1250], 'FontWeight', 'Bold');
end
...
title(site_name + ' - mean weighted STD vs. m/n ratio'); xlabel('m/n'); ylabel('Mean weighted STD [m]'); legend('east','west', 'Location', 'NW');